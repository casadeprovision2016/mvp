...
Domain service for notification-service in Cotai MVP.

## Overview

notification-service is a microservice responsible for [define specific domain responsibility].
...
```bash
docker build -f docker/Dockerfile -t cotai-notification-service:latest .
docker run -p 50051:50051 cotai-notification-service:latest
```
...
```bash
helm install notification-service ./charts/ -f charts/values-dev.yaml -n dev
```
