...
Domain service for auth-service in Cotai MVP.

## Overview

auth-service is a microservice responsible for [define specific domain responsibility].
...
```bash
docker build -f docker/Dockerfile -t cotai-auth-service:latest .
docker run -p 50051:50051 cotai-auth-service:latest
```
...
```bash
helm install auth-service ./charts/ -f charts/values-dev.yaml -n dev
```
